...
function Navbar({ currentPage, onNavigate, isLoggedIn, variant = "light" }) {
      ...
      <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8">
        <div className="flex min-h-[44px] flex-wrap items-center justify-between gap-4 lg:flex-nowrap">
          ...
          >
            Ekspans
            <span
              className={isLight ? "font-medium text-sky-600" : "font-light"}
            >
              Expo
            </span>
          </a>
          ...
          <nav className="order-3 flex w-full flex-wrap items-center justify-center gap-x-6 gap-y-3 text-[0.98rem] md:order-2 md:w-auto md:text-[1rem]">
            {navigationItems.map(({ label, page }) => (
              <button
                ...
                type="button"
                onClick={() => {
                  if (page === "exhibitions" && !isLoggedIn) {
          ...
          <div className="order-2 hidden items-center gap-4 md:order-3 md:flex">
            {isLoggedIn ? (
              <>
                <Bell className="cursor-pointer text-slate-700 hover:text-slate-900" />
                <Settings className="cursor-pointer text-slate-700 hover:text-slate-900" />
                <div className="w-9 h-9 rounded-full bg-gray-300 cursor-pointer border border-3 border-white " />
              </>
            ) : (
              authActions.map(({ label, action, isPrimary }) => (
                <button
                  ...
                  className={
                    isPrimary
                      ? "rounded-full border border-slate-900 bg-slate-950 px-6 py-2 text-white"
                      : "text-slate-700"
                  }
                >
                  {label}
